...
interface BookmarkContextValue {
  ...
  isLineBookmarked: (page: number, line: number) => boolean;
...
export function BookmarkProvider({ children }: { children: ReactNode }) {
    ...
    (item: {
      ...
      setBookmarks((prev) => {
        ...
        const exists = prev.some(
          (b) => b.page === item.page && (item.line ? b.line === item.line : !b.line)
        );
        if (exists) return prev;

        const padPage = String(item.page).padStart(3, '0');
        const linePart = item.line ? `.${String(item.line).padStart(2, '0')}` : '';
        const id = `bm-${padPage}${linePart}-${Date.now().toString(36)}`;
        ...
        const newBookmark: BookmarkItem = {
          ...
          title: item.title || (item.line ? `Page ${padPage}, Line ${item.line}` : `Joyce Page ${padPage}`),
  ...
  const toggleBookmark = useCallback(
    (page: number, line?: number, title?: string, excerpt?: string) => {
      setBookmarks((prev) => {
        const existingIdx = prev.findIndex(
          (b) => b.page === page && (line ? b.line === line : !b.line)
        );
        ...
        if (existingIdx >= 0) {
          ...
          const newBookmark: BookmarkItem = {
            ...
            page,
            line,
            title: title || (line ? `Page ${padPage}, Line ${line}` : `Joyce Page ${padPage}`),
            excerpt,
            createdAt: new Date().toISOString(),
          };
  ...
  const isPageBookmarked = useCallback(
    (page: number) => {
      return bookmarks.some((b) => b.page === page && !b.line);
    },
    [bookmarks]
  );
  ...
  const isLineBookmarked = useCallback(
    (page: number, line: number) => {
      return bookmarks.some((b) => b.page === page && b.line === line);
    },
    [bookmarks]
  );
