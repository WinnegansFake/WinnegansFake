...
import { GITHUB_REPO_URL, type DissertationDefinition } from '@winnegans/core';

export interface DissertationViewerProps {
  content: string;
...
export function DissertationViewer({
  content,
  dissertation,
  basePath = '',
  backHref = '/dissertations',
  onBack,
  LinkComponent,
}: DissertationViewerProps) {
  ...
  const markdownComponents = useMemo(() => ({
    ...
    img: ({ node, src, alt, ...props }: any) => {
      ...
      if (resolvedSrc.startsWith('figures/')) {
        resolvedSrc = basePath ? `${basePath}/${resolvedSrc}` : `/${resolvedSrc}`;
          ...
          <div className="mb-4">
            {onBack ? (
              <button
                type="button"
                onClick={onBack}
                className="inline-flex items-center space-x-1.5 text-xs font-mono text-slate-400 hover:text-emerald-400 transition-colors cursor-pointer"
              >
