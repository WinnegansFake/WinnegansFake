...
import { PageLine, getBookAndChapterInfo, getWork, getWorkDivision } from '@winnegans/core';
import { getMapperForWork } from '@winnegans/epub-reader';
...
export class BrowserEpubService {
  ...
  public async parseFromUrl(url: string, name?: string, workId: string = 'finnegans-wake'): Promise<number> {
    ...
    const derivedName = name || url.split('/').pop()?.split('?')[0] || 'work.epub';
  ...
  public async parseFile(
    ...
    // 4. Map pages using registered mapper or default Joycean/sequential mapping
    const work = getWork(workId);
    if (work.id === 'finnegans-wake') {
      // Finnegans Wake archive mapping
      ...
      // General work mapping
      let pageCounter = work.startPage || 1;
      for (const href of this.spine) {
        ...
        try {
          const textContent = await fileEntry.async('text');
          if (textContent.length < 150) continue;
          ...
          this.pageMap.set(pageCounter, {
            href,
            text: cleanText,
          });
          pageCounter++;
          if (pageCounter > (work.totalPages || 1000)) break;
        } catch {
          continue;
        }
      }
  ...
  public async getPage(pageNumber: number): Promise<ParsedEpubPage | null> {
    ...
    const info = getBookAndChapterInfo(pageNumber, this.currentWorkId);
  ...
  public segmentLines(rawText: string, pageNum: number): PageLine[] {
    let text = rawText
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>')
      .replace(/&amp;/g, '&')
